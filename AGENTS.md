...
- `lib/_wasi.mjs` — minimal WASI preview1 shim. Implements only the ~20 syscalls ripgrep actually imports (`fd_read`, `fd_write`, `fd_readdir`, `path_open`, `path_filestat_get`, `fd_filestat_get`, `fd_fdstat_get`, `fd_prestat_*`, `args_*`, `environ_*`, `clock_time_get`, `random_get`, `proc_exit`, `sched_yield`, `poll_oneoff` stub, `fd_close`, `fd_tell`). Backed by `node:fs` sync APIs so it works on Node, Bun, and Deno uniformly. `proc_exit` throws a `WASIExit` that `start()` catches to return the exit code.
